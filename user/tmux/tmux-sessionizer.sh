...
create_new_session() {
    ...
    # Rename the first window to "nvim"
    tmux rename-window -t $selected_name:1 "nvim"
    # Create a left pane that takes 60% of the space
    tmux split-window -h -p 70 -t $selected_name:1 -c $selected
    # Move to the right pane
    tmux select-pane -t $selected_name:1.2
    # Open nvim in the right pane
    tmux send-keys -t $selected_name:1.2 "nvim ." C-m
    # Select and zoom the right pane (nvim)
    tmux select-pane -t $selected_name:1.2
